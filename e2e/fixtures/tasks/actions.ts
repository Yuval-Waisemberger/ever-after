...
export type FixtureTask = { id: string; title: string; notes: string | null; category: string | null; due_date: string | null; priority: "high" | "medium" | "low"; status: TaskStatus };
...
export async function getOwnedWedding() { return { wedding_date: "2026-09-12" }; }
export async function saveTask(_state: ActionState, form: FormData): Promise<ActionState> {
  ...
  const task = { id: p.data.id || crypto.randomUUID(), title: p.data.title, notes: p.data.notes, category: p.data.category, due_date: p.data.dueDate, priority: p.data.priority, status: p.data.status };
