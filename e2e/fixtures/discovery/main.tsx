...
import ProfilePage from "@/app/vendors/[slug]/page";
...
function Fixture() {
  ...
  useEffect(() => { if(view === "directory-page") void DirectoryPage({params:Promise.resolve({}),searchParams:Promise.resolve({category:"photography-content"})}).then(setPage); if(view === "profile") void ProfilePage({params:Promise.resolve({slug:vendor.slug}),searchParams:Promise.resolve({})}).then(setPage); if(view === "my") void MyVendorsPage({params:Promise.resolve({}),searchParams:Promise.resolve({})}).then(setPage); },[revision,view]);
  const content = <>
    ...
    {view === "profile" || view === "my" ? page : view === "actions" ? <main className="mx-auto max-w-xl p-6"><h1 className="mb-6 font-display text-3xl">Willow Studio</h1><VendorStatusActions businessName={vendor.businessName} vendorId={vendor.id} currentStatus={fixture.status} isSaved={fixture.saved} returnTo="/" /></main> : <main className="public-theme directory-page mx-auto max-w-7xl p-6">
      ...
      <section className="marketplace-results"><VendorFiltersForm filters={{page:1,category:"photography-content"}} subcategories={[{slug:"wedding-photographers",name:"Wedding photographers",categorySlug:"photography-content"}]} />
        <VendorResults>{[0,1,2,3].map(i=><VendorCard key={i} vendor={{...vendor,id:vendor.id+i,businessName:i===0?vendor.businessName:`Willow Studio ${i+1}`,isSaved:fixture.saved,recommendation:i===0?vendor.recommendation:null}} canSave={i!==3} />)}</VendorResults>
      </section>
    </main>}
  </>;
