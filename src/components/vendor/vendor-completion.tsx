...
const steps = [
  ...
  { label: "Add at least 2 photos", target: "gallery", icon: Images },
];
...
export function VendorCompletion({ profileId, percentage, nextSteps, imageCount }: { profileId: string; percentage: number; nextSteps: string[]; imageCount: number }) {
  ...
  return <section ref={node} className="vendor-panel vendor-completion ea-surface ea-surface--blush" aria-labelledby="completion-title">
    ...
    <p className="mt-2 text-sm text-ink-soft">{steps.length - nextSteps.length} of {steps.length} steps</p>
    ...
    <ul className="vendor-checklist">{steps.map(({ label, target, icon: Icon }) => { const complete = !nextSteps.includes(label); return <li key={label} data-step={label} data-complete={complete} data-next={!complete && label === nextSteps[0]}><Link href={`/vendor/profile#${target}`}><span className="vendor-check" aria-hidden="true">{complete ? <Check /> : <Icon />}</span><span className="vendor-check-label">{label}</span>{!complete && label === nextSteps[0] ? <span className="vendor-dashboard-next" aria-hidden="true">NEXT</span> : null}<span className="sr-only">{complete ? " — complete, edit" : " — add details"}</span><ArrowUpRight className="ml-auto size-3.5 shrink-0 opacity-50" aria-hidden="true" /></Link></li>; })}</ul>
