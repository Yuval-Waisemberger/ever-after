...
export default async function VendorProfilePage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-5 py-8 sm:px-8 lg:px-10 lg:py-12">
      <PageHeader eyebrow="Vendor" title={profile ? "My Business Profile" : "Vendor Profile Setup"} description={profile ? "This information shapes your public profile and deterministic matching. Only you can edit it." : "Add your business details to create your private Vendor Profile. You can publish it when you are ready."} />
      <div className="mt-8"><VendorProfileForm profile={profile} taxonomy={taxonomy} /></div>
      {profile ? <section className="mt-7 rounded-2xl border bg-paper p-5 sm:p-7"><h2 className="font-display text-2xl">Gallery</h2><p className="mt-2 text-sm text-ink-soft">JPG, PNG, or WebP. Maximum 5 MB per image.</p><div className="mt-5"><VendorImageUpload vendorId={profile.id} /></div>{profile.vendor_images?.length ? <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{profile.vendor_images.map((image) => { const url = image.external_url ?? (image.storage_path ? `${supabaseUrl}/storage/v1/object/public/vendor-media/${image.storage_path}` : null); if (!url) return null; return <figure key={image.id} className="overflow-hidden rounded-xl border"><div className="relative aspect-[4/3]"><Image src={url} alt={image.alt_text ?? "Vendor gallery image"} fill sizes="33vw" className="object-cover" /></div><figcaption className="flex items-center justify-between gap-3 px-3 py-2 text-xs text-ink-soft"><span className="line-clamp-1">{image.alt_text || "No alt text"}</span><form action={deleteVendorImage}><input type="hidden" name="imageId" value={image.id} /><button aria-label="Delete image" className="text-red-700"><Trash2 className="size-3.5" /></button></form></figcaption></figure>; })}</div> : null}</section> : null}
    </main>
  );
}
