...
as $$
  select case
    when split_part(object_name, '/', 1) ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
      then split_part(object_name, '/', 1)::uuid = (select auth.uid())
    else false
  end;
$$;
...
comment on column public.couple_vendors.is_saved is
  'Independent bookmark flag. Lifecycle status may simultaneously be contacted, considering, booked, or rejected.';
