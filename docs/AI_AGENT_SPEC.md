...
## Phase 6B — bounded research foundation (mocked validation only)

This section supersedes earlier phase snapshots describing research as interface-only. The final authorized live procedural research revalidation returned a safe partial result. The OpenAI path now exposes twelve custom functions: the unchanged ten internal READ tools plus `research_current_wedding_info` and `get_market_benchmark`. The Local provider remains unchanged. The legacy pure eligibility helper and `live: false` contract descriptors are not execution switches or live-validation claims.

Research follows a separate `research/server.ts` executor, never the internal database executor. It reserves one invocation across both names per logical turn, even after failure. `OpenAIResearchAdapter` makes one Responses request, no nested loop, with only `web_search` and `max_tool_calls: 1`. The local type is `ResponseCreateParamsNonStreaming & { max_tool_calls: 1 }`; no cast, suppression, SDK patch or dependency upgrade is used. The official HTTP parameter bounds built-in calls, not pages or sources. An injected HTTP transport test verifies actual SDK serialization. The existing client has zero retries.

The main loop still has four model rounds, final round answer-only, six requested custom calls and a 30-second whole-turn deadline. Research consumes the custom-call budget and at most one additional bounded adapter Responses request; it does not consume another admission. The adapter receives an AbortSignal capped by the executor to min(15 seconds, remaining turn time). Returned usage is aggregated with main-round usage when complete. Missing usage is not represented as zero. No short-window throttle is required or restored.

Current-info requests add a closed aspect enum: general, required_documents, fees, timing, registration_steps, eligibility, procedure. Existing normalization excludes raw prompts/history/identity/notes, drops offerDescription, coarsens event date and guest scale, and strips event context for procedures. Supplied vendor quote context is distinct from the private wedding budget. Stable advice needs no research; current procedures and market claims may use their respective research tools.

Source URLs must match returned web-search action sources or SDK URL citation annotations and safe HTTPS domains without credentials or custom ports. Authority is server-owned: gov.il and its subdomains are official; additional domains require review in SOURCE_POLICY. Unknown domains remain commercial_article regardless of provider labels. No additional market publisher has been presumed reviewed. Benchmarks require matching structured package/currency context; a typical range additionally requires two reviewed independent publishers and suitable source classes. Same-publisher domains do not manufacture independence. Data remains in the original minor monetary units.

Freshness is conservative: sources older than 180 days are insufficient; dates are never invented. Even recent or undated provider-extracted material remains partial because current applicability and comparability have not been independently established. This foundation does not certify natural-language factual truth or universal current market averages. Future live QA must assess usefulness of these intentionally qualified results.

The server issues a turn-local fingerprinted research receipt containing normalized results, source references and finding IDs, protected by object identity plus a tamper-detection signature. Final citations must match validated finding/range source URLs. External evidence is generated by server attestation; copied/modified receipts or fabricated provider evidence fail. Existing Couple/Marketplace protections remain. Unavailable or insufficient results have no usable current facts; instructions require limitation disclosure and stable general guidance. No research receipts are stored in a new table.

Safe diagnostics add only research stage, approved names, result state, source/finding counts and available usage. No queries, URLs, titles, findings or raw provider errors are logged. All validation is local/mocked; no live search, admission, Frankfurt contact, credentials, billing or deployment was used. Known admitted usage remains 13.
